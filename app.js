...
import {
  ...
  deleteDoc, onSnapshot, query, orderBy, where, setDoc, getDoc, runTransaction
} from './firebase-config.js';
import { PAKET_LIST, BIAYA_PENDAFTARAN, hitungBiayaDaftar, getKurikulum, getMateriForPertemuan } from './kurikulum.js';

// ================================================================
//  STATE MANAGEMENT — 3 states: loading | data | error
// ================================================================
const AppState = {
  siswa: { loading: true, error: null },
  instruktur: { loading: true, error: null },
  pengeluaran: { loading: true, error: null }
};

function setLoading(key, isLoading) { AppState[key].loading = isLoading; }
function setError(key, err) { AppState[key].error = err; }

function showTableState(tbodyId, colSpan, state, emptyMsg='Belum ada data.') {
  const el = document.getElementById(tbodyId);
  if (!el) return;
  if (state === 'loading') {
    el.innerHTML = `<tr><td colspan="${colSpan}" class="empty-row"><i class="fas fa-spinner fa-spin" style="margin-right:8px;color:#9ca3af"></i>Memuat data...</td></tr>`;
  } else if (state === 'error') {
    el.innerHTML = `<tr><td colspan="${colSpan}" class="empty-row" style="color:var(--red)"><i class="fas fa-exclamation-triangle" style="margin-right:8px"></i>Gagal memuat. <button onclick="listenSiswa()" style="background:none;border:none;color:var(--green-dark);cursor:pointer;font-weight:700;text-decoration:underline">Coba lagi</button></td></tr>`;
  } else if (state === 'empty') {
    el.innerHTML = `<tr><td colspan="${colSpan}" class="empty-row">${emptyMsg}</td></tr>`;
  }
}
// ================================================================
//  STATE VARIABLES
...
window.doLogin = async function() {
  ...
  if (selectedLoginRole==='siswa') {
    try {
      ...
      sessionStorage.setItem('djUser', JSON.stringify(currentUser));
      enterApp();
      // Force password change jika masih pakai password default (tanggal lahir)
      if (!data.passwordChanged) {
        setTimeout(() => openModal('modalGantiPass'), 800);
      }
      return;
    } catch(e) { errEl.textContent='Error: '+e.message; errEl.classList.remove('hidden'); return; }
  }
...
function buildTabs() {
  ...
  if (currentUser.role==='admin') {
    ...
    // Fix: sembunyikan semua sub-panel kecuali keuangan
    document.querySelectorAll('#adminHome .sub-panel').forEach(p => p.classList.add('hidden'));
    document.getElementById('adminHome_keuangan')?.classList.remove('hidden');
    setTimeout(()=>{ renderLaporan(); listenPengeluaran(); },500);
...
function listenSiswa() {
  // Show loading state
  showTableState('tbodyKeuangan', 7, 'loading');
  showTableState('tbodyBiodata', 7, 'loading');

  onSnapshot(
    query(collection(db,'siswa'), orderBy('tglDaftar','desc')),
    snap => {
      setError('siswa', null);
    ...
    err => {
      ...
      // Tampilkan error yang informatif ke user
      showTableState('tbodyKeuangan', 7, 'error');
      showTableState('tbodyBiodata', 7, 'error');
      const errMsg = err.code === 'permission-denied'
        ? '⚠️ Akses database ditolak. Rules Firebase mungkin sudah expired. Hubungi admin untuk memperbarui rules.'
        : `⚠️ Gagal koneksi ke database: ${err.message}`;
      showToast(errMsg, 'error');
      // Tampilkan banner error
      const banner = document.createElement('div');
      banner.style.cssText = 'background:#fee2e2;color:#991b1b;padding:12px 16px;border-radius:8px;margin-bottom:14px;font-size:13px;font-weight:600;';
      banner.innerHTML = `<i class="fas fa-exclamation-triangle"></i> ${errMsg} <button onclick="location.reload()" style="margin-left:12px;background:#991b1b;color:white;border:none;padding:4px 10px;border-radius:4px;cursor:pointer;font-size:12px">Refresh</button>`;
      const keuPanel = document.getElementById('adminHome_keuangan');
      if (keuPanel && !document.getElementById('errorBanner')) {
        banner.id = 'errorBanner';
        keuPanel.prepend(banner);
      }
    }
  );
}
...
function renderKeuanganTable(list) {
  ...
  tbody.innerHTML = list.map(s=>{
    ...
    const biayaDaftar = s.biayaPendaftaran ?? 0;
    return `<tr>
      ...
      <td>
        ${formatRp(s.biaya)}
        ${biayaDaftar > 0
          ? `<div style="font-size:10px;color:#991b1b">+${formatRp(biayaDaftar)} biaya daftar</div>`
          : `<div style="font-size:10px;color:#166534">✅ Gratis daftar</div>`}
      </td>
...
function renderLedger(list) {
  ...
  list.forEach(s=>{
    ...
    const tglValid = tgl.match(/^\d{4}-\d{2}/) ? tgl : today();
    const key = tglValid.substring(0,7); // YYYY-MM
    if (!groups[key]) {
      const [y,m] = key.split('-');
      const tglObj = new Date(Number(y), Number(m)-1, 1);
      groups[key] = { label: tglObj.toLocaleDateString('id-ID',{month:'long',year:'numeric'}), items:[], total:0 };
    }
...
// ================================================================
//  LUPA AKSES — Reset Password & Cari Username
// ================================================================
let lupaPassAttempts = 0;
let lupaUserAttempts = 0;
const MAX_ATTEMPTS = 3;

const WA_ADMIN = 'https://wa.me/6285156783819?text=';

window.switchLupaTab = function(tab) {
  const isPass = tab === 'password';
  document.getElementById('panelLupaPass').style.display = isPass ? 'block' : 'none';
  document.getElementById('panelLupaUser').style.display = isPass ? 'none' : 'block';
  document.getElementById('tabLupaPass').style.background = isPass ? 'var(--green-dark)' : 'var(--gray-100)';
  document.getElementById('tabLupaPass').style.color = isPass ? 'white' : 'var(--gray-600)';
  document.getElementById('tabLupaUser').style.background = isPass ? 'var(--gray-100)' : 'var(--green-dark)';
  document.getElementById('tabLupaUser').style.color = isPass ? 'var(--gray-600)' : 'white';
};

window.resetPassword = async function() {
  const hp = document.getElementById('lupaHP').value.trim();
  const nama = document.getElementById('lupaNama').value.trim();
  const tglLahir = document.getElementById('lupaTglLahir').value;
  const errEl = document.getElementById('lupaPassError');
  const successEl = document.getElementById('lupaPassSuccess');
  const waEl = document.getElementById('lupaPassWA');
  ...
  try {
    // Cari siswa berdasarkan No HP
    const snap = await getDocs(query(collection(db,'siswa'), where('hp','==',hp)));

    if (snap.empty) {
      lupaPassAttempts++;
      errEl.textContent = `No HP tidak terdaftar! (${lupaPassAttempts}/${MAX_ATTEMPTS} percobaan)`;
      errEl.classList.remove('hidden');
    } else {
      const data = snap.docs[0].data();
      const siswaId = snap.docs[0].id;

      // Verifikasi nama (case-insensitive) DAN tanggal lahir
      const namaMatch = data.nama?.toLowerCase().trim() === nama.toLowerCase().trim();
      const tglMatch = data.tglLahir === tglLahir;

      if (namaMatch && tglMatch) {
        // ✅ Verifikasi berhasil — reset password ke default
        await updateDoc(doc(db,'siswa',siswaId), {
          passwordChanged: false, // reset flag agar wajib ganti password lagi
          passwordBaru: null
        });
        ...
        successEl.innerHTML = `
          <i class="fas fa-check-circle"></i> Password berhasil direset!<br><br>
          <strong>Login sekarang dengan:</strong><br>
          • Username: <code style="background:#f0fdf4;padding:2px 6px;border-radius:4px">${hp}</code><br>
          • Password: <code style="background:#f0fdf4;padding:2px 6px;border-radius:4px">${tglLahir.split('-').reverse().join('')}</code>
          <span style="font-size:11px;color:#166534;display:block;margin-top:6px">(Tanggal lahir kamu: ddmmyyyy)</span>
          <p style="margin-top:8px;font-size:11px;color:#4b5563">Setelah login, kamu akan diminta membuat password baru.</p>
        `;
        lupaPassAttempts = 0;
        ...
        // ❌ Verifikasi gagal
        lupaPassAttempts++;
        const sisaPercobaan = MAX_ATTEMPTS - lupaPassAttempts;
        if (lupaPassAttempts >= MAX_ATTEMPTS) {
          errEl.textContent = 'Verifikasi gagal terlalu banyak kali.';
          errEl.classList.remove('hidden');
          // Tampilkan tombol WA Admin
          waEl.style.display = 'block';
          const waMsg = encodeURIComponent(`Halo Admin Dirgantara Jaya, saya lupa password akun kursus saya.\n\nNo HP: ${hp}\nNama: ${nama}\n\nMohon bantu reset password saya. Terima kasih 🙏`);
          document.getElementById('lupaPassWABtn').href = WA_ADMIN + waMsg;
          btn.style.display = 'none';
        } else {
          errEl.textContent = `Nama atau tanggal lahir tidak cocok! Sisa percobaan: ${sisaPercobaan}`;
          errEl.classList.remove('hidden');
        }
      }
    }
    ...
    if (btn.style.display !== 'none') {
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-key"></i> Reset Password Saya';
    }
  }
};

window.cariUsername = async function() {
  const nama = document.getElementById('lupaUserNama').value.trim();
  const tgl = document.getElementById('lupaUserTgl').value;
  const errEl = document.getElementById('lupaUserError');
  const successEl = document.getElementById('lupaUserSuccess');
  const waEl = document.getElementById('lupaUserWA');
  ...
  try {
    // Cari berdasarkan tanggal lahir dulu (lebih spesifik)
    const snap = await getDocs(query(collection(db,'siswa'), where('tglLahir','==',tgl)));

    if (!snap.empty) {
      // Cocokkan dengan nama
      const matches = snap.docs.filter(d =>
        d.data().nama?.toLowerCase().trim() === nama.toLowerCase().trim()
      );
      ...
      if (matches.length > 0) {
        ...
        waEl.style.display = 'none';
        successEl.style.display = 'block';
        document.getElementById('lupaUserResult').innerHTML = `
          <div style="display:flex;flex-direction:column;gap:6px">
            <div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid rgba(0,0,0,.06)">
              <span>Nama</span><strong>${esc(data.nama)}</strong>
            </div>
            <div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid rgba(0,0,0,.06)">
              <span>Username (No HP)</span>
              <strong style="color:var(--green-dark);font-size:16px">${esc(data.hp)}</strong>
            </div>
            <div style="display:flex;justify-content:space-between;padding:5px 0">
              <span>Paket</span><strong>${esc(data.paketLabel||'-')}</strong>
            </div>
          </div>
          <p style="margin-top:10px;font-size:11px;color:#4b5563">
            Password default kamu: <strong>${(data.tglLahir||'').split('-').reverse().join('')}</strong> (tanggal lahir ddmmyyyy)
          </p>
        `;
        lupaUserAttempts = 0;
        return;
    ...
    lupaUserAttempts++;
    if (lupaUserAttempts >= MAX_ATTEMPTS) {
      errEl.textContent = 'Data tidak ditemukan setelah beberapa percobaan.';
      errEl.classList.remove('hidden');
      waEl.style.display = 'block';
      const waMsg = encodeURIComponent(`Halo Admin Dirgantara Jaya, saya lupa username/No HP akun kursus saya.\n\nNama: ${nama}\nTanggal Lahir: ${tgl}\n\nMohon bantu temukan akun saya. Terima kasih 🙏`);
      document.getElementById('lupaUserWABtn').href = WA_ADMIN + waMsg;
    } else {
      errEl.textContent = `Nama atau tanggal lahir tidak cocok! (${lupaUserAttempts}/${MAX_ATTEMPTS} percobaan)`;
      errEl.classList.remove('hidden');
    }
...
window.resetLupaAkses = function() {
  // Reset semua state modal
  lupaPassAttempts = 0;
  lupaUserAttempts = 0;
  ['lupaHP','lupaNama','lupaTglLahir','lupaUserNama','lupaUserTgl'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
  ['lupaPassError','lupaUserError'].forEach(id => {
    document.getElementById(id)?.classList.add('hidden');
  });
  ['lupaPassSuccess','lupaUserSuccess','lupaPassWA','lupaUserWA'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.style.display = 'none';
  });
  const btn = document.getElementById('btnResetPass');
  if (btn) { btn.style.display = 'flex'; btn.disabled = false; btn.innerHTML = '<i class="fas fa-key"></i> Reset Password Saya'; }
  switchLupaTab('password');
};

// ================================================================
// ================================================================
window.simpanPasswordBaru = async function() {
  const passBaru = document.getElementById('passBaruInput').value.trim();
  const konfirm = document.getElementById('passBaruKonfirm').value.trim();
  const errEl = document.getElementById('gantiPassError');

  // Validasi JavaScript (business rules)
  ...
  // Pastikan bukan password default (tanggal lahir)
  const userData = siswaCacheList.find(s => s.id === currentUser?.id);
  if (userData) {
    const defaultPass = (userData.tglLahir||'').split('-').reverse().join('');
    if (passBaru === defaultPass) {
      errEl.textContent = 'Password baru tidak boleh sama dengan tanggal lahir kamu!';
      errEl.classList.remove('hidden'); return;
    }
  }
  errEl.classList.add('hidden');
  ...
  try {
    // Simpan password baru ke Firestore (hashed dengan btoa sederhana — production sebaiknya pakai bcrypt di backend)
    await updateDoc(doc(db, 'siswa', currentUser.id), {
      passwordBaru: btoa(passBaru), // encode sederhana
      passwordChanged: true
    });
    closeModal('modalGantiPass');
    showToast('✅ Password berhasil diperbarui! Akun kamu lebih aman sekarang.');
  } catch(e) {
...
window.lewatiGantiPass = function() {
  closeModal('modalGantiPass');
  showToast('⚠️ Disarankan segera ganti password untuk keamanan akun!', 'error');
};

// ================================================================
//  ACID TRANSACTION — Pembayaran (mencegah race condition)
// ================================================================
window.simpanBayar = async function() {
  ...
  const btnSimpan = document.querySelector('#modalBayar .btn-primary');
  if (btnSimpan) { btnSimpan.disabled = true; btnSimpan.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Menyimpan...'; }

  try {
    // Gunakan Firestore Transaction untuk mencegah race condition
    await runTransaction(db, async (transaction) => {
      const siswaRef = doc(db, 'siswa', currentBayarId);
      const siswaSnap = await transaction.get(siswaRef);

      if (!siswaSnap.exists()) throw new Error('Data siswa tidak ditemukan!');

      const currentData = siswaSnap.data();
      const newTerbayar = Number(currentData.terbayar || 0) + jumlah;
      const biaya = Number(currentData.biaya || 0);

      // Validasi: tidak boleh melebihi biaya
      if (newTerbayar > biaya) {
        throw new Error(`Total bayar (${formatRp(newTerbayar)}) melebihi biaya kursus (${formatRp(biaya)})!`);
      }

      transaction.update(siswaRef, {
        terbayar: newTerbayar,
        // Catat histori pembayaran
        riwayatBayar: [...(currentData.riwayatBayar || []), {
          jumlah,
          tgl: today(),
          jam: nowTime(),
          ket: document.getElementById('modalBayarKet').value.trim() || 'Pembayaran'
        }]
      });
    });

    showToast(`✅ Bayar ${formatRp(jumlah)} berhasil dicatat!`);
    closeModal('modalBayar');
  } catch(e) {
    showToast(`❌ ${e.message}`, 'error');
  } finally {
    if (btnSimpan) { btnSimpan.disabled = false; btnSimpan.innerHTML = '<i class="fas fa-save"></i> Simpan'; }
  }
};
...
function renderVerifikasi(list) {
  ...
  el.innerHTML = menunggu.map(s => {
    ...
    const nominalDP   = s.nominalDP || s.nominalTransfer || 0;
    const biayaDaftar = s.biayaPendaftaran || 0;
    const isLunas     = s.statusBayar === 'lunas';
    ...
    return `<div class="verif-card ${s.statusAkun}">
      ...
      <div class="verif-card-body">
        <div class="verif-info-grid">
          ...
          <div class="verif-info-item"><span>Biaya Kursus</span><strong style="color:#1a3d2b">${formatRp(s.biaya)}</strong></div>
          ...
          <div class="verif-info-item">
            ...
            <strong style="color:${isLunas?'#166534':'#854d0e'}">
              ${formatRp(nominalDP)} <span style="font-size:10px">(${isLunas?'LUNAS':'CICILAN'})</span>
            </strong>
          </div>
          <div class="verif-info-item">
            <span>Biaya Pendaftaran</span>
            <strong style="color:${biayaDaftar>0?'#991b1b':'#166534'}">
              ${biayaDaftar>0 ? formatRp(biayaDaftar) : '✅ GRATIS'}
            </strong>
          </div>
          ${!isLunas ? `<div class="verif-info-item"><span>Sisa Kursus</span><strong style="color:#991b1b">${formatRp(s.biaya-nominalDP)}</strong></div>` : ''}
        ...
        <div class="verif-actions">
          <button class="btn-primary" onclick="aktifkanAkun('${s.id}','${esc(s.nama)}',${nominalDP},${biayaDaftar})">
...
window.aktifkanAkun = async function(id, nama, nominalDP, biayaDaftar) {
  const s = siswaCacheList.find(x => x.id === id);
  const totalBayar = (nominalDP||0) + (biayaDaftar||0);
  const isLunas = (nominalDP||0) >= (s?.biaya||0);
  if (!confirm(
    `Aktifkan akun "${nama}"?\n\n` +
    `Nominal dibayar: ${formatRp(nominalDP||0)}\n` +
    `Biaya pendaftaran: ${(biayaDaftar||0)>0 ? formatRp(biayaDaftar) : 'GRATIS'}\n` +
    `Total diterima: ${formatRp(totalBayar)}\n` +
    `Status: ${isLunas ? 'LUNAS' : 'CICILAN'}`
  )) return;
  try {
    await updateDoc(doc(db,'siswa',id), {
      statusAkun: 'aktif',
      terbayar: nominalDP||0,
      tglAktif: today(),
      riwayatBayar: [{
        jumlah: totalBayar,
        tgl: today(), jam: nowTime(),
        ket: `Pembayaran awal via ${s?.metodeBayar||''}${(biayaDaftar||0)>0?' + biaya daftar':' (gratis pendaftaran)'}`
      }]
    });
    showToast(`✅ Akun "${nama}" berhasil diaktifkan!`);
    if (s && confirm('Kirim notifikasi WhatsApp ke siswa?')) {
      const waUrl = `https://wa.me/${(s.hp||'').replace(/^0/,'62')}?text=${encodeURIComponent(
        `Halo ${s.nama}! 🎉 Akun kursus mengemudi Dirgantara Jaya kamu AKTIF!\n\n` +
        `📱 Login: remarkable-marigold-ccebb8.netlify.app\n` +
        `👤 Username: ${s.hp}\n` +
        `🔑 Password: ${(s.tglLahir||'').split('-').reverse().join('')}\n` +
        `📦 Paket: ${s.paketLabel||''}\n` +
        `💰 Status: ${isLunas?'LUNAS':'CICILAN'}\n\n` +
        `Selamat belajar! 🚗 Info: 0851-5678-3819`
      )}`;
...
function updateDaftarPreview() {
  ...
  const biayaDaftar = hitungBiayaDaftar(key, dp);
  const totalBayarPertama = dp + biayaDaftar;
  const statusPembayaran = dp >= paket.harga ? 'LUNAS' : 'CICILAN';
  ...
  content.innerHTML = `
    ...
    <div class="preview-row"><span>Biaya Kursus</span><span>${formatRp(paket.harga)}</span></div>
    <div class="preview-row"><span>DP / Uang Muka</span><span>${formatRp(dp)}</span></div>
    <div class="preview-row"><span>Status Pembayaran</span>
      <span style="color:${statusPembayaran==='LUNAS'?'#166534':'#854d0e'};font-weight:800">${statusPembayaran}</span>
    </div>
    <div class="preview-row">
      <span>Biaya Pendaftaran</span>
      <span style="color:${biayaDaftar===0?'#166534':'#991b1b'};font-weight:700">
        ${biayaDaftar === 0 ? '✅ GRATIS (Bayar Lunas)' : formatRp(biayaDaftar)}
      </span>
    </div>
    <div class="preview-row" style="border-top:2px solid var(--green-dark);margin-top:4px;padding-top:8px">
      <span style="font-weight:700">Total Bayar Sekarang</span>
      <span style="font-size:16px;font-weight:800;color:var(--green-dark)">${formatRp(totalBayarPertama)}</span>
    </div>
...
window.daftarSiswa = async function() {
  ...
  // VALIDASI: HTML5 handles basic, JS handles business rules
  if (!nama || !hp || !tglLahir || !paketKey) {
    showMsg(msg,'error','❌ Nama, No HP, Tanggal Lahir, dan Paket wajib diisi!'); return;
  }
  if (!/^08\d{8,11}$/.test(hp)) {
    showMsg(msg,'error','❌ Format No HP tidak valid! Contoh: 08123456789'); return;
  }
  const paket = PAKET_LIST[paketKey];
  if (dp > paket.harga) {
    showMsg(msg,'error',`❌ DP (${formatRp(dp)}) tidak boleh melebihi biaya kursus (${formatRp(paket.harga)})!`); return;
  }

  try {
    // Cek duplikat HP sebelum simpan
    const cekDuplikat = await getDocs(query(collection(db,'siswa'), where('hp','==',hp)));
    if (!cekDuplikat.empty) {
      showMsg(msg,'error',`❌ No HP ${hp} sudah terdaftar!`); return;
    }

    const paket = PAKET_LIST[paketKey];
    const biayaDaftar = hitungBiayaDaftar(paketKey, dp);
    const totalBayarAwal = dp + biayaDaftar;
    ...
    await addDoc(collection(db,'siswa'), {
      ...
      biaya: paket.harga,
      biayaPendaftaran: biayaDaftar,
      terbayar: dp,
      tglDaftar, catatan, absen: [],
      statusAkun: 'aktif',
      passwordChanged: false,
      riwayatBayar: totalBayarAwal > 0 ? [{
        jumlah: totalBayarAwal,
        tgl: tglDaftar,
        jam: nowTime(),
        ket: `DP${biayaDaftar > 0 ? ` + Biaya Pendaftaran (${formatRp(biayaDaftar)})` : ' (Lunas - Gratis Pendaftaran)'}`
      }] : []
    });
    showMsg(msg,'success',`✅ "${nama}" berhasil didaftarkan!${biayaDaftar > 0 ? ` Biaya pendaftaran: ${formatRp(biayaDaftar)}` : ' Biaya pendaftaran: GRATIS (lunas)'}`);
    resetFormDaftar();
...
window.showSubTab = function(parentId, subKey, btn) {
  ...
  if (!parent) return;
  // Sembunyikan semua sub-panel dalam parent ini
  parent.querySelectorAll('.sub-panel').forEach(p => p.classList.add('hidden'));
  // Tampilkan yang dipilih
  const target = document.getElementById(`${parentId}_${subKey}`);
  if (target) target.classList.remove('hidden');
  // Update tab aktif
  parent.querySelectorAll('.sub-tab').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');
  // Muat data spesifik jika perlu
  if (subKey === 'pengeluaran') { listenPengeluaran(); }
  if (subKey === 'laporan') { renderLaporan(); }
  if (subKey === 'reminder') { renderReminder(siswaCacheList); }
  if (subKey === 'verifikasi') { renderVerifikasi(siswaCacheList); }
};
