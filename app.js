...
window.simpanPengeluaran = async function() {
  ...
  const fotoBukti = document.getElementById('pFoto').value.trim();
  ...
  if (!fotoBukti || !fotoBukti.startsWith('http')) { showMsg(msg,'error','❌ Link foto bukti wajib diisi! Upload ke Google Drive dulu.'); return; }

  try {
    await addDoc(collection(db,'pengeluaran'),{ tgl, kategori, keterangan, jumlah, fotoBukti, createdAt: new Date().toISOString() });
